...
def draw(Phi, Phi2, N, L):
    Phi = np.reshape(Phi, N * L, 'F')
    Phi2 = np.reshape(Phi2, N * L, 'F')

    with open('faza_enc.pkl', 'wb') as f:
        pickle.dump([Phi, Phi2], f)

    # Rysujemy dla fazy
    t = np.arange(0, len(Phi), 1)
    ...
    plt.plot(Phi2, lw=1)
    splt.tick_params(axis='both', labelsize='12')
    splt.set_title("Faza sygnału stego_coverAudio po osadzeniu informacji metodą kodowania fazowego")
    splt.set_xlabel("Próbki")
    splt.set_ylabel("Faza [rad]")
    plt.show(fig)
    plt.close(fig)
...
def encode(signal, text):
    ...
    wavfile.write('../data/data_from_embedding/' + 'stego4_' + song_name, samplerate, audio)
