...
signal = "C:/Users/aslod/Documents/Ważne foldery/STUDIA/7 semestr/Praca inzynierska/Stegography/MyStego/src/data/coverAudio.wav"
...
add_zeros = np.zeros(len(data) - len(output_signal))
output_signal = np.append(output_signal, add_zeros)

out = data
input_signal = data[:, 0] + 0.02 * output_signal
...
decoded_message = input_signal[:len(carrier_signal)] * coarse_acquisition_code_signal * carrier_signal
fgef = np.average(np.reshape(decoded_message, (message_code.size, -1)), 1)
wat = np.reshape(decoded_message, (message_code.size, -1))
decoded_message_code = binarize(np.average(np.reshape(decoded_message, (message_code.size, -1)), 1), [0, 1]),
my_info = decoded_message_code[0]
my_message = fromBits(my_info)
message_code = np.reshape(message_code, (len(message_code), 1))
wait = " blablabla"
